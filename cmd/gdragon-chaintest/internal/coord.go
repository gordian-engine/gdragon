...
type Genesis struct {
	Ed25519PubKeys []ed25519.PublicKey
}
...
func (c *Coordinator) addPubKey(k ed25519.PublicKey) {
	if len(k) != ed25519.PublicKeySize {
		panic(fmt.Errorf(
			"illegal key size (need %d, got %d)",
			ed25519.PublicKeySize, len(k),
		))
	}
	...
	for _, have := range c.pubKeys {
		if k.Equal(have) {
			panic(fmt.Errorf("key %x registered twice", k))
		}
	}

	c.pubKeys = append(c.pubKeys, k)
}
...
func (c *Coordinator) getGenesis() Genesis {
	...
	defer c.mu.Unlock()
	return Genesis{
		Ed25519PubKeys: slices.Clone(c.pubKeys),
	}
}
